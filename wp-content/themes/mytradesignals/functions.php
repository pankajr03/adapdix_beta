...

{
	...
	wp_enqueue_script('mytradesignals-jquery-3-4-1', 'https://code.jquery.com/jquery-3.4.1.min.js', array(), '1.0', true);

	wp_enqueue_script('mytradesignals-slick-carousel-1-5-9', 'https://cdnjs.cloudflare.com/ajax/libs/slick-carousel/1.5.9/slick.js', array(), '1.0', true);

	wp_enqueue_script('mytradesignals-bootstrap', get_template_directory_uri() . '/js/bootstrap.min.js', array(), '1.0', true);

	wp_enqueue_script('mytradesignals-video-popup', get_template_directory_uri() . '/js/video-popup.js', array(), '1.0', true);

	wp_enqueue_script('mytradesignals-owl-carousel-min', get_template_directory_uri() . '/js/owl.carousel.min.js', array(), '1.0', true);
	...
	// Bootstrap CSS 
    wp_enqueue_style('mytradesignals-home-bootstrap', get_template_directory_uri() . '/css/bootstrap.min.css', array(), '202009071');
	wp_enqueue_style('mytradesignals-home-typekit', 'https://use.typekit.net/lbu0drw.css', array(), '202009072');
	    
	//Custom CSS
    wp_enqueue_style('mytradesignals-home', get_template_directory_uri() . '/css/style.css', array(), '202009073');
	
    wp_enqueue_style('mytradesignals-home-responsive', get_template_directory_uri() . '/css/responive.css', array(), '202009074');
	
	wp_enqueue_style('mytradesignals-home-owl-carousel', get_template_directory_uri() . '/css/owl.carousel.min.css', array(), '202009075');
	wp_enqueue_style('mytradesignals-home-owl-theme-default', get_template_directory_uri() . '/css/owl.theme.default.min.css', array(), '202009076');
		
    wp_enqueue_style('mytradesignals-home-slick', 'https://cdnjs.cloudflare.com/ajax/libs/slick-carousel/1.5.9/slick.css', array(), '20130727');
    wp_enqueue_style('mytradesignals-home-slick-theme', 'https://cdnjs.cloudflare.com/ajax/libs/slick-carousel/1.5.9/slick-theme.css', array(), '202009078');
	
	wp_enqueue_style('mytradesignals-home-custom', get_template_directory_uri() . '/style.css', array(), '202009079');
    /*
	...
	wp_enqueue_style('mytradesignals-responsive', get_template_directory_uri() . '/css/responsive.css', array(), '20170315');
    /*
...
{
	/** 
	 * Start Adapdix Only Widget
	 */
    // company-info-content
	register_sidebar(array(

        'name' => __('Resources Information', 'mytradesignals'),
	...
	register_sidebar(array(
        ...
        'name' => __('Company Address Information', 'mytradesignals'),

        'id' => 'company-address-content',
	...
	register_sidebar(array(
        ...
        'name' => __('Company Google Map', 'mytradesignals'),

        'id' => 'company-gmap-content',
	...
	register_sidebar(array(
        ...
        'name' => __('Partner Portion', 'mytradesignals'),
	...
	register_sidebar(array(
        ...
        'name' => __('Featured Contents', 'mytradesignals'),

        'id' => 'featured-info-content',
	...
	register_sidebar(array(
        ...
        'name' => __('Our Team', 'mytradesignals'),
	...
	// board-advisors-content
	register_sidebar(array(

        'name' => __('Board & Advisors', 'mytradesignals'),

        'id' => 'board-advisors-content',
	...
	register_sidebar(array(
        ...
        'name' => __('Contact Sidebar', 'mytradesignals'),
 ...
 register_sidebar(array(
        ...
        'name' => __('Public Front Page Opt-in Form', 'mytradesignals'),

        'id' => 'public-frontpage-optin-form',

        'description' => __('This Widget will display on the public front page ', 'mytradesignals'),
...
{ global $pmpro_level, $current_user;
   ...
   if($current_user->membership_level->ID < 1)
        $classes[] = $current_user->membership_level->ID.'ch mbhide-responsive-menu';
...
add_action('wp_footer', 'mbcustom_css');
function mbcustom_css(){
echo'<style type="text/css"> body.mbhide-responsive-menu #responsive-menu-button, 
body.mbhide-responsive-menu #responsive-menu-container{display:none!important;}</style>';
}
...

{
    ...
    $message = "<h1 class='login-message'><i class='fa fa-lock'></i> Login To Your Account:
...
/* send and email to admin and user once a rank is achived */

 add_action('affwp_ranks_pre_set_affiliate_rank', 'sendEmailToAdminAndUser_onceRankAchived', 9, 3);

 function sendEmailToAdminAndUser_onceRankAchived($affiliate_id, $rank_id, $old_rank_id) {
...
// Get the current rank's data
        ...
        $current_rank = get_rank_by_id($rank_id);

        // Get the last rank's data

        $last_rank = get_rank_by_id($old_rank_id);

        $earned_ranks = affwp_ranks_get_affiliate_earned_ranks($affiliate_id);

        /* subscribe and send mail if the current rank is not in the earned ranks Array */

        if (is_array($earned_ranks) && !in_array($rank_id, $earned_ranks)) {
            ...
            /* Subscribe user to the new mailchimp list */

//                $file_path=get_bloginfo('url') . '/wp-content/plugins/affiliatewp-mailchimp-add-on/classes/api/MailChimp.php';

//                echo $file_path;

            require_once ABSPATH . '/wp-content/plugins/affiliatewp-mailchimp-add-on/classes/api/MailChimp.php';
            ...
            if ($rank_id == 5762) {//silver

                $mailchimp_list = 'add7cac668';

            } elseif ($rank_id == 394) {//Gold

                $mailchimp_list = '6d9e34a2ed';

                //Remove from silver and add to Gold

                $unsub = $MailChimp->call('lists/unsubscribe', array(

                         'id' => 'add7cac668',

                        'email' =>array('email' => $to), 
            ...
            } elseif ($rank_id == 7983) {//Platinum

                $mailchimp_list = '627c35a936';

                //Remove from Gold and add to Platinum
            ...
            } elseif ($rank_id == 6231) {//Double Platinum

                $mailchimp_list = '0b9a75d312';

                //Remove from Platinum and add to Double Platinum
            ...
            } elseif ($rank_id == 8879) {//Triple Platinum

                $mailchimp_list = '75516e19a4';

                //Remove from Double Platinum and add to Triple Platinum
            ...
            $headers = array(
                ...
                'From: info@mobbyd.com',

                'CC:' . $admin_email,

            );

            $subject = 'New Rank Achived';

            $body = 'Dear user you are promoted to the heigher Rank, Your last Rank was ' . $last_rank[0]['name'] . ' And your current Rank is ' . $current_rank[0]['name'] . '';
...
if ( class_exists( 'AffiliateWP_Multi_Level_Marketing' ) ) {
	...
	function affwp_mlm_remove_upline_aff_data( $aff_data = array(), $affiliate_id = 0 ) {
		...
		if ( is_array( $upline ) && in_array( $affiliate_id, $upline ) ) {
...
/**
 * Get the recurring rate for an affiliate based on their rank
 * 
 * @since 1.0.3
 */
function mobbyd_affwp_ranks_get_recurring_rank_rate( $rate, $affiliate_id, $context ) {
...
add_filter( 'affwp_get_recurring_referral_rate', 'mobbyd_affwp_ranks_get_recurring_rank_rate', 10, 3 );

/**
 * Get the recurring rate type for an affiliate based on their rank
 * 
 * @since 1.0.3
 */
function mobbyd_affwp_ranks_get_recurring_rank_rate_type( $rate_type, $affiliate_id, $context ) {
...
add_filter( 'affwp_get_recurring_referral_rate_type', 'mobbyd_affwp_ranks_get_recurring_rank_rate_type', 10, 3 );
...
// Creating the widget 
class ciw_widget extends WP_Widget {
	...
	function __construct() {
		...
		__('Company Info Widget', 'ciw_widget_domain'), 
  
		// Widget description
		array( 'description' => __( 'Sample widget based on Company Info', 'ciw_widget_domain' ), ) 
		);
	}
	...
	public function widget( $args, $instance ) {
		...
		if ( ! empty( $title ) )
			...
			echo __( 'Hello, Company Info!', 'ciw_widget_domain' );
	...
	public function form( $instance ) {
		...
		$title = __( 'New title', 'ciw_widget_domain' );
		}
		// Widget admin form
		?>
		<p>
			...
			<div class=" custom-container">
				<div class="row header-banner lines-bg align-content-center header-responive m-0">
					<div class="col-md-6 banner-text p-0 banner-mob-part">
						...
						<p class="lead text-white mb-3 hide-m"><?php echo $subtitle?></p>
						<p><a class="action-btn mb-5 margintp20 hide-m" href="<?php echo $button_link?>"><?php echo $button_text?></a></p>
					</div>
					<div class="col-md-7  banner-text banner-text-mob">
						<p class="lead text-white mb-3 hide-dt"><?php echo $subtitle?></p>
						<p><a class="action-btn mb-5 hide-dt" href="<?php echo $button_link?>"><?php echo $button_text?></a></p>
					</div>
				</div>
			</div>
		</div>
...
function pageBannerCompany($args) {
	$title = $args['title'];
	$subtitle = $args['subtitle'];
	$button_text = $args['button_text'];
	$button_link = $args['button_link'];
	...
	<header>
        <div class="company-banner-bg company-banner-mob devlop-bg" style="background-image: url(<?php echo $photo?>)">
            <div class="custom-container">
            <div
                class="row header-banner lines-bg align-items-center header-responive m-0"
            >
                <div class="col-md-5 banner-text p-0 banner-mob-comp">
                <h1 class="mb-3"><?php echo $title?></h1>
                <h4 class="hide-m" id="over100">
                    <?php echo $subtitle?>
                </h4>
                <p class="lead text-white mb-5 hide-m">
                    <?php echo $button_text?>
                </p>
                </div>
                <div class="col-md-7 banner-text banner-text-mob">
                <h4 class="hide-dt"><?php echo $subtitle?></h4>
                <p class="lead text-white mb-5 hide-dt">
                    <?php echo $button_text?>
                </p>
                </div>
            </div>
...
function pageBannerHome($args) {
	...
	<header>
        <div class="custom-container">
            <div
            class="row header-banner strip-bg align-items-center header-responive m-0 hide-m homepage-header"
            >
            <div class="col-md-6 banner-text p-0">
                <h1 class="mb-3"><?php echo $title?></h1>
                <p class="lead text-white mb-4 pr-lg-5">
                <?php echo $subtitle?>
                </p>
                <p class="lead text-white mb-4 pr-lg-5 f-w500">
                    <?php echo $button_text?></p>
                <p>
                <a
                    class="action-btn margintp20"
                    href="<?php echo $button_link?>"
                    >Learn more</a
                >
                <a class="action-btn-purple margintp20" href="#" id="get_a_demo"
                    >GET A DEMO</a
                >
                </p>
            </div>
            <div class="col-md-6 text-right banner-img">
                <img id="img-fluid-w-75" class="img-fluid w-75" src="<?php echo $photo?>" />
                <p></p>
                <div id="get_a_demo_content">
                <h3>See Adapdix EdgeOps platform live</h3>
                <p>
                
                <?php echo do_shortcode('[contact-form-7 id="992" title="See Adapdix EdgeOps platform live"]');?>
                </p>
                </div>
            </div>
            </div>
            <div
            class="row header-banner strip-bg align-items-center header-responive m-0 hide-dt homepage-header-mob"
            >
            <div class="col-md-6 banner-text">
                <h1><?php echo $title?></h1>
                <div class="banner-img-mobile text-center">
                <img class="img-fluid w-75" src="<?php echo $photo?>" />
                </div>
                <p class="lead text-white mb-4 pr-lg-5"> <?php echo $subtitle?></p>
                <p class="lead text-white mb-4 pr-lg-5 f-w500">
                    <?php echo $button_text?>
                </p>
                <p>
                <a class="action-btn margintp20"
                    href="<?php echo $button_link?>"
                    >Learn more</a
                >
                </p>
...
function pageBannerEdgeOps($args) {
	$title = $args['title'];
    $subtitle = $args['subtitle'];
    $button_text = $args['button_text'];
	$button_link = $args['button_link'];
	...
	<header>
        <div class="custom-container">
            ...
            <div class="col-md-6 text-center banner-img">
