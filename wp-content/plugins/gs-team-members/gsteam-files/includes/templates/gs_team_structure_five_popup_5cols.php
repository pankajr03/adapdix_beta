...
$output .= '<div class="row clearfix gs_team" id="gs_team'.get_the_id().'">';
	...
	if ( $GLOBALS['gs_team_loop']->have_posts() ) {
		...
		while ( $GLOBALS['gs_team_loop']->have_posts() ) {
			...
			$output .= '<a class="gs_team_pop open-popup-link" href="#gs_team_popup_'.get_the_id().'" data-effect="">'; // Start Link
	        ...
	        $output .= '<div class="single-member-name-desig cbp-so-side cbp-so-side-right">'; // start Name Desig
