	...
	if ( $GLOBALS['gs_team_loop']->have_posts() ) {
		...
		while ( $GLOBALS['gs_team_loop']->have_posts() ) {
			...
			$output .= '<div itemscope="" itemtype="http://schema.org/Person">'; //Start sehema
				...
				$output .= '<div class="single-member cbp-so-side cbp-so-side-right">'; // start single member

					$output .= '<a class="" href="'. $gs_member_desc_link .'">'; // Start Link
			        ...
			        $output .= '<div class="gs_team_overlay"><i class="fa fa-eye"></i></div>';
