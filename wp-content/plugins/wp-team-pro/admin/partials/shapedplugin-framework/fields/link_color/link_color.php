 ...
 * @version 2.0
 */
if ( ! class_exists( 'SPF_Field_link_color' ) ) {
	class SPF_Field_link_color extends SPF_Fields {
		...
		public function render() {
			...
			$color_props = array(
				'color'   => esc_html__( 'Normal', 'spf' ),
				'hover'   => esc_html__( 'Hover', 'spf' ),
				'active'  => esc_html__( 'Active', 'spf' ),
				'visited' => esc_html__( 'Visited', 'spf' ),
				'focus'   => esc_html__( 'Focus', 'spf' ),
			);
			...
			foreach ( $color_props as $color_prop_key => $color_prop_value ) {
				...
				if ( ! empty( $args[ $color_prop_key ] ) ) {
					...
					echo '<div class="spf--title">' . $color_prop_value . '</div>';
					echo '<input type="text" name="' . $this->field_name( '[' . $color_prop_key . ']' ) . '" value="' . $value[ $color_prop_key ] . '" class="spf-color"' . $default_attr . $this->field_attributes() . '/>';
					echo '</div>';
