 ...
 * @version 2.0
 */
if ( ! class_exists( 'SPF_Field_group' ) ) {
	class SPF_Field_group extends SPF_Fields {
		...
		public function render() {
			...
			$args = wp_parse_args(
				$this->field, array(
					...
					'min'                    => 0,
					'fields'                 => array(),
					'button_title'           => esc_html__( 'Add New', 'spf' ),
			...
			if ( ! empty( $this->parent ) && preg_match( '/' . preg_quote( '[' . $this->field['id'] . ']' ) . '/', $this->parent ) ) {
				...
				echo '<i class="spf-cloneable-remove spf-confirm fa fa-times" data-confirm="' . esc_html__( 'Are you sure to delete this item?', 'spf' ) . '"></i>';
				echo '</div>';
				...
				foreach ( $this->field['fields'] as $field ) {
					...
					spf::field( $field, $field_default, '_nonce', 'field/group', $field_parent );
				...
				if ( ! empty( $this->value ) ) {
					...
					foreach ( $this->value as $value ) {
						...
						foreach ( $this->field['fields'] as $field ) {
							...
							spf::field( $field, $field_value, $field_unique, 'field/group', $field_parent );
				...
				echo '<div class="spf-cloneable-alert spf-cloneable-max">' . esc_html__( 'You can not add more than', 'spf' ) . ' ' . $args['max'] . '</div>';
				echo '<div class="spf-cloneable-alert spf-cloneable-min">' . esc_html__( 'You can not remove less than', 'spf' ) . ' ' . $args['min'] . '</div>';
