 ...
 * @version 2.0
 */
if ( ! class_exists( 'SPF_Field_spacing' ) ) {
	class SPF_Field_spacing extends SPF_Fields {
		...
		public function render() {
			...
			if ( ! empty( $args['all'] ) ) {
				...
				echo '<input type="number" name="' . $this->field_name( '[all]' ) . '" value="' . $value['all'] . '"' . $placeholder . ' class="spf-number" max="' . $value['max'] . '" min="' . $value['min'] . '" step="' . $value['step'] . '" />';
				...
				foreach ( $properties as $property ) {
					...
					echo '<input type="number" name="' . $this->field_name( '[' . $property . ']' ) . '" value="' . $value[ $property ] . '"' . $placeholder . ' class="spf-number" max="' . $value['max'] . '" min="' . $value['min'] . '" step="' . $value['step'] . '" />';
					echo ( count( $args['units'] ) === 1 && ! empty( $args['unit'] ) ) ? '<span class="spf--label spf--label-unit">' . $args['units'][0] . '</span>' : '';
					echo '</div>';
