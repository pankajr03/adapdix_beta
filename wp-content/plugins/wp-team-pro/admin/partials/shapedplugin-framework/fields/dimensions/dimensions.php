 ...
 * @version 2.0
 */
if ( ! class_exists( 'SPF_Field_dimensions' ) ) {
	class SPF_Field_dimensions extends SPF_Fields {
		...
		public function render() {
			...
			$args = wp_parse_args(
				$this->field, array(
					...
					'width_placeholder'  => esc_html__( 'width', 'spf' ),
					'height_placeholder' => esc_html__( 'height', 'spf' ),
					'width'              => true,
					'height'             => true,
					'unit'               => true,
					'show_units'         => true,
			...
			if ( ! empty( $args['width'] ) ) {
				...
				echo ( ! empty( $args['width_icon'] ) ) ? '<span class="spf--label spf--label-icon">' . $args['width_icon'] . '</span>' : '';
				echo '<input type="text" name="' . $this->field_name( '[width]' ) . '" value="' . $value['width'] . '"' . $placeholder . ' class="spf-number" />';
			...
			if ( ! empty( $args['height'] ) ) {
				...
				echo '<div class="spf--input">';
				echo ( ! empty( $args['height_icon'] ) ) ? '<span class="spf--label spf--label-icon">' . $args['height_icon'] . '</span>' : '';
				echo '<input type="text" name="' . $this->field_name( '[height]' ) . '" value="' . $value['height'] . '"' . $placeholder . ' class="spf-number" />';
				echo ( count( $args['units'] ) === 1 && ! empty( $args['unit'] ) ) ? '<span class="spf--label spf--label-unit">' . $args['units'][0] . '</span>' : '';
				echo '</div>';
