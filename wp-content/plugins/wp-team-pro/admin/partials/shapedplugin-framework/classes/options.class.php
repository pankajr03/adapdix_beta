...
if ( ! class_exists( 'SPF_Options' ) ) {
	class SPF_Options extends SPF_Abstract {
		...
		public $args         = array(
			...
			// framework title.
			'framework_title'         => 'ShapedPlugin Framework',
			'framework_class'         => '',
		...
		public function set_options() {
			...
			if ( wp_verify_nonce( spf_get_var( 'spf_options_nonce' . $this->unique ), 'spf_options_nonce' ) ) {
				...
				if ( ! empty( $transient['spf_import_data'] ) ) {
					...
					$this->notice = esc_html__( 'Success. Imported backup options.', 'spf' );
					...
					$this->notice = esc_html__( 'Default options restored.', 'spf' );
					...
					$this->notice = esc_html__( 'Default options restored for only this section.', 'spf' );
				...
				if ( empty( $this->notice ) ) {
					$this->notice = esc_html__( 'Settings saved.', 'spf' );
				}
			}
		...
		public function add_options_html() {
			...
			echo ( $has_nav && $this->args['show_all_options'] ) ? '<div class="spf-expand-all" title="' . esc_html__( 'show all options', 'spf' ) . '"><i class="fa fa-outdent"></i></div>' : '';

			echo ( $this->args['show_search'] ) ? '<div class="spf-search"><input type="text" name="spf-search" placeholder="' . esc_html__( 'Search option(s)', 'spf' ) . '" autocomplete="off" /></div>' : '';

			echo '<div class="spf-buttons">';
			echo '<input type="submit" name="' . $this->unique . '[_nonce][save]" class="button button-primary spf-save' . $ajax_class . '" value="' . esc_html__( 'Save', 'spf' ) . '" data-save="' . esc_html__( 'Saving...', 'spf' ) . '">';
			echo ( $this->args['show_reset_section'] ) ? '<input type="submit" name="spf_transient[reset_section]" class="button button-secondary spf-reset-section spf-confirm" value="' . esc_html__( 'Reset Section', 'spf' ) . '" data-confirm="' . esc_html__( 'Are you sure to reset this section options?', 'spf' ) . '">' : '';
			echo ( $this->args['show_reset_all'] ) ? '<input type="submit" name="spf_transient[reset]" class="button button-secondary spf-warning-primary spf-reset-all spf-confirm" value="' . esc_html__( 'Reset All', 'spf' ) . '" data-confirm="' . esc_html__( 'Are you sure to reset all options?', 'spf' ) . '">' : '';
			echo '</div>';
			...
			foreach ( $this->pre_sections as $section ) {
				...
				if ( ! empty( $section['fields'] ) ) {
					...
					foreach ( $section['fields'] as $field ) {
						...
						spf::field( $field, $value, $this->unique, 'options' );

					}
				} else {

					echo '<div class="spf-no-option spf-text-muted">' . esc_html__( 'No option provided by developer.', 'spf' ) . '</div>';
			...
			if ( ! empty( $this->args['show_footer'] ) ) {
				...
				echo '<div class="spf-buttons">';
				echo '<input type="submit" name="spf_transient[save]" class="button button-primary spf-save' . $ajax_class . '" value="' . esc_html__( 'Save', 'spf' ) . '" data-save="' . esc_html__( 'Saving...', 'spf' ) . '">';
				echo ( $this->args['show_reset_section'] ) ? '<input type="submit" name="spf_transient[reset_section]" class="button button-secondary spf-reset-section spf-confirm" value="' . esc_html__( 'Reset Section', 'spf' ) . '" data-confirm="' . esc_html__( 'Are you sure to reset this section options?', 'spf' ) . '">' : '';
				echo ( $this->args['show_reset_all'] ) ? '<input type="submit" name="spf_transient[reset]" class="button button-secondary spf-warning-primary spf-reset-all spf-confirm" value="' . esc_html__( 'Reset All', 'spf' ) . '" data-confirm="' . esc_html__( 'Are you sure to reset all options?', 'spf' ) . '">' : '';
				echo '</div>';
