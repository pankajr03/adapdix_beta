	...
	abstract class SPF_Fields extends SPF_Abstract {
		...
		public static function field_data( $type = '', $term = false, $query_args = array() ) {
			...
			switch ( $type ) {
				...
				case 'wptp_member_social_icons':
					$social_icon_list_one = array(
						'facebook'       => 'Facebook',
						'twitter'        => 'Twitter',
						'linkedin'       => 'LinkedIn',
						'skype'          => 'Skype',
					...
					$social_icon_list_two = apply_filters(
						'sp_wptp_member_social_icons', array()
					);
					$social_icon_list     = array_merge( $social_icon_list_one, $social_icon_list_two );

					if ( ! empty( $social_icon_list ) ) {
						foreach ( $social_icon_list as $social_icon_key => $social_icon_name ) {
							$options[ $social_icon_key ] = $social_icon_name;
						}
					}
				...
				case 'img_sizes':
					...
					$options['original'] = __( 'Original uploaded Image', 'wp-team-pro' );
					$options['custom']   = __( 'Set Custom size', 'wp-team-pro' );
					break;
