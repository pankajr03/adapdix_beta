 ...
 * @version 1.0.0
 */
if ( ! class_exists( 'SPF_Profile_Options' ) ) {
	class SPF_Profile_Options extends SPF_Abstract {
		...
		public function render_profile_form_fields( $profileuser ) {
			...
			echo '<div class="spf spf-profile spf-onload' . $class . '">';

			wp_nonce_field( 'spf_profile_nonce', 'spf_profile_nonce' . $this->unique );
			...
			foreach ( $this->sections as $section ) {
				...
				if ( ! empty( $section['fields'] ) ) {
					foreach ( $section['fields'] as $field ) {
						...
						SPF::field( $field, $this->get_meta_value( $profile_id, $field ), $this->unique, 'profile' );
		...
		public function save_profile( $user_id ) {
			...
			if ( wp_verify_nonce( spf_get_var( 'spf_profile_nonce' . $this->unique ), 'spf_profile_nonce' ) ) {
