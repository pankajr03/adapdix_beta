 ...
 * @version 2.0
 */
if ( ! class_exists( 'SPF_Metabox' ) ) {
	class SPF_Metabox extends SPF_Abstract {
		...
		public function add_meta_box_content( $post, $callback ) {
			...
			foreach ( $this->sections as $section ) {
				...
				if ( ! empty( $section['fields'] ) ) {
					...
					foreach ( $section['fields'] as $field ) {
						...
						spf::field( $field, $this->get_meta_value( $field ), $this->unique, 'metabox' );
			...
			if ( ! empty( $this->args['show_restore'] ) ) {
				...
				echo '<span class="button spf-button-restore">' . esc_html__( 'Restore', 'spf' ) . '</span>';
				echo '<span class="button spf-button-cancel">' . sprintf( '<small>( %s )</small> %s', esc_html__( 'update post for restore ', 'spf' ), esc_html__( 'Cancel', 'spf' ) ) . '</span>';
				echo '</label>';
				echo '</div>';
		...
		public function save_meta_box( $post_id ) {
			...
			if ( ! wp_verify_nonce( spf_get_var( 'spf_metabox_nonce' . $this->unique ), 'spf_metabox_nonce' ) ) {
				return $post_id;
			}
