 ...
 * @version 2.0
 */
if ( ! class_exists( 'SPF_Shortcoder' ) ) {
	class SPF_Shortcoder extends SPF_Abstract {
		...
		public $args         = array(
			...
			'class'          => '',
			'gutenberg'      => array(
				'title'       => 'SPF Shortcodes',
				'description' => 'SPF Shortcode Block',
				'icon'        => 'screenoptions',
				'category'    => 'widgets',
		...
		public function __construct( $key, $params = array() ) {
			...
			if ( ! empty( $this->args['show_in_editor'] ) ) {
				...
				SPF::$shortcode_instances[] = wp_parse_args(
					array(
						'hash'     => md5( $key ),
				...
				if ( SPF::is_active_plugin( 'elementor/elementor.php' ) ) {
						add_action( 'elementor/editor/before_enqueue_scripts', array( 'spf', 'add_admin_enqueue_scripts' ), 20 );
		...
		public function add_shortcode_modal() {
			...
			?>
			<div id="spf-modal-<?php echo $this->unique; ?>" class="wp-core-ui spf-modal spf-shortcode<?php echo $single_usage . $class; ?>" data-modal-id="<?php echo $this->unique; ?>" data-nonce="<?php echo wp_create_nonce( 'spf_shortcode_nonce' ); ?>">
		...
		public function get_shortcode() {
			...
			if ( ! empty( $shortcode_key ) && wp_verify_nonce( spf_get_var( 'nonce' ), 'spf_shortcode_nonce' ) ) {
				...
				if ( ! empty( $section ) ) {
					...
					if ( ! empty( $section['fields'] ) && $view !== 'repeater' ) {
						...
						foreach ( $section['fields'] as $field ) {
							...
							SPF::field( $field, $field_default, $shortcode, 'shortcode' );
					...
					if ( ! empty( $repeatable_fields ) ) {
						...
						$button_title    = ( ! empty( $section['button_title'] ) ) ? ' ' . $section['button_title'] : esc_html__( 'Add one more', 'spf' );
						...
						foreach ( $repeatable_fields as $field ) {
							...
							spf::field( $field, $field_default, $inner_shortcode . '[0]', 'shortcode' );
				...
				echo '<div class="spf-field spf-text-error">' . esc_html__( 'Error: Nonce verification has failed. Please try again.', 'spf' ) . '</div>';
			}
		...
		public static function once_editor_setup() {
			...
			if ( function_exists( 'register_block_type' ) ) {
				add_action( 'init', array( 'spf_Shortcoder', 'add_guteberg_block' ) );
			}

			if ( spf_wp_editor_api() ) {
				add_action( 'media_buttons', array( 'spf_Shortcoder', 'add_media_buttons' ) );
			}
		...
		public static function add_guteberg_block() {
			...
			wp_register_script( 'spf-gutenberg-block', SPF::include_plugin_url( 'assets/js/spf-gutenberg-block.js' ), array( 'wp-blocks', 'wp-editor', 'wp-element', 'wp-components' ) );

			wp_localize_script( 'spf-gutenberg-block', 'spf_gutenberg_blocks', SPF::$shortcode_instances );

			foreach ( SPF::$shortcode_instances as $hash => $value ) {

				register_block_type(
					'spf-gutenberg-block/block-' . $hash, array(
