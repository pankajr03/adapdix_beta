...
class Advanced_Categories_Widget_Views
{
	...
	public static function list_item( $term, $instance, $categories, $echo = true )
	{
		...
		$team_meta_arr = get_term_meta( $term_id, 'featured_image_id' );
		$thumb_img = '';
		if ( array($team_meta_arr) > 0 ) {
			$thumb_img = wp_get_attachment_image(	$team_meta_arr[0] );
				...
				<?php do_action( 'acatw_item_top', $term, $instance ); ?>
					...
					<div class="item">
						  <div class="feature-boxs">
							<span class="box-head-title"><?php  if( ! empty( $thumb_div ) ) { echo $thumb_div; }; ?></span>
							<h5 class="mb-3"><?php echo $term->name?></h5> 
								<p class=""><?php echo $item_desc; ?></p>
						</div>
					</div>
...
/**
	 ...
	 * @return string $html Closing tag element for the list item.
	 */
	public static function list_item_mobile( $term, $instance, $categories, $echo = true )
	{
		$item_desc  = Advanced_Categories_Widget_Utils::get_term_excerpt( $term, $instance );
		$item_id    = Advanced_Categories_Widget_Utils::get_item_id( $term, $instance );
		$item_class = Advanced_Categories_Widget_Utils::get_item_class( $term, $instance );
		...
		do_action( 'acatw_item_before', $term, $instance );
		?>
			<div class="col-md-6 col-lg-3 col-lx-3 mb-3 hide-m">
				...
				<?php do_action( 'acatw_item_top', $term, $instance ); ?>
					...
					<div class="feature-boxs">
						...
						<?php 
						/*
						 printf( '<h5 class="mb-3"><a href="%s" rel="bookmark">%s</a></h3>',
							esc_url( get_term_link( $term ) ),
							sprintf( __( '%s', 'advanced-categories-widget'), $term->name )
						);
						*/
						echo ( '<h5 class="mb-3">'.$term->name.'</h5>');
						?>
